...
ruminate ask "什么是过拟合" --effort thorough   # 多角度检索 + HyDE
...
Query
  │
  ├── [fast] 直接向量检索
  ├── [balanced] HyDE：LLM 生成假设文档 → 嵌入 → 向量检索
  └── [thorough] 查询扩展：LLM 生成 2-3 变体 → 各自检索 → RRF 融合
  │
...
| 查询扩展   | `--effort balanced`：HyDE 假设文档；`--effort thorough`：多查询变体 + RRF |
