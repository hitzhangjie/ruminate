...
│                 原始资料层（Evidence / 事实真相）    │
│  文章、论文、笔记、网页、代码指针……                  │
│  只读快照 — 不被 LLM 改写，可回退核对                 │
└──────────────────────────────────────────────────┘
                       ↓ AI 编译（有损蒸馏）
┌──────────────────────────────────────────────────┐
│                 知识 Wiki 层（Synthesis / 理解真相） │
│  摘要页、实体页、概念页、对比页、综合索引……           │
│  完全由 AI 生成和维护；高密度，但不能替代原文         │
└──────────────────────────────────────────────────┘
                       ↓ 受控于
┌──────────────────────────────────────────────────┐
│                 Schema 配置层                      │
...
传统检索管线（混合检索、查询扩展、HyDE、MMR、重排序）已基本完整，继续优化的边际收益递减。未来投入集中在 **Agent 驱动的主动探索**——它的天花板远高于固定管线：

- **HTTP Server + REST API**：`ruminate serve` 启动 API 服务
- **Web 前端**：Wiki 浏览、AI 对话、摄入管理、知识图谱
- **Agent 自评与迭代**：ReAct 结束时 LLM 自评答案质量，不足则继续探索、请求用户补充信息、或回退到外部搜索引擎
- **Agent 工具扩展**：LSP 符号跳转、git log/blame 追溯变更历史、跨仓库探索
